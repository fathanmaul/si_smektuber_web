    ...
    <div class="card rounded p-6 drop-shadow-lg bg-white mt-4 lg:w-3/5">
        ...
        <form action="{{ route('ppdb.daftar.put', $registration->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="flex flex-col gap-2 mt-4">
                ...
                <select name="school_year" class="select select-info border max-w-xs w-full rounded p-2" required>
                    ...
                    @foreach($schoolYears as $year)
                        <option value="{{ $year->id }}" {{ $registration->school_year == $year->id ? 'selected' : '' }}>
                            {{ $year->school_year }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex flex-col gap-2 mt-6">
                ...
                <input type="file" class="file-input file-input-bordered @error('poster') input-error @enderror w-full border-gray-400 rounded" name="poster" />
                @error('poster')
                    <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex flex-col gap-2 mt-4">
                ...
                <input type="date" name="date_start"id="date_start"class="input @error('date_start') input-error @enderror border-gray-400 rounded"
