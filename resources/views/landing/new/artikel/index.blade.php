      ...
      <div class="max-h-[30rem] bg-slate-100 py-8 px-10">
          <div class="container mx-auto flex flex-col-reverse gap-8 md:gap-0 md:flex-row items-center justify-between">
              <div class="flex-1 h-full mt-4 md:mt-0">
                  <h1 class="text-xl text-center md:text-left md:text-xl font-bold text-slate-800">Artikel</h1>
                  <p class="text-sm mt-4 text-center md:text-left md:text-base text-slate-800">Daftar Artikel Terbaru</p>
              </div>
          </div>
      </div>
                  ...
                  @foreach ($artikel as $item)
                      <div
                          ...
                          <div class="flex-1 flex flex-col gap-2 w-full lg:max-w-[60%] px-4 pb-8 lg:py-4">
                              <h1 class="font-bold overflow-hidden text-ellipsis whitespace-nowrap">
                                  {{ $item->title ?? '-' }}</h1>
                              <div class="flex flex-row gap-6">
                                  <div class="flex flex-row items-center gap-1">
                                      <ion-icon name="calendar"></ion-icon>
                                      <p class="text-sm text-slate-800">{{ Carbon::parse($item->created_at)->format('d M Y') }}</p>
                                  </div>
                              ...
                              <a href="{{ route('landing.artikel.detail', $item->slug) }}"
                                  class="text-sm hover:underline hover:text-blue-600">Lihat Selengkapnya...</a>
                          </div>
                      </div>
                  @endforeach
                  {{ $artikel->links() }}
